...
public:
     ...
     */
    explicit TcpSocket(const in_port_t &port, const std::string &ip = "0.0.0.0") : _sockfd(Sockfd()),
        _addr_helper{port, ip} {
    }

    explicit TcpSocket(const sockaddr_in &addr) : _sockfd(Sockfd()) {
        _addr_helper.SetAddr(addr);
    }

    explicit TcpSocket(const InetAddr &inet_addr) : _sockfd(Sockfd()), _addr_helper(inet_addr) {
    }


    void Bind() const {
        sockaddr_in temp{_addr_helper.GetAddr()};
        if (const int ret = bind(_sockfd, reinterpret_cast<sockaddr *>(&temp), sizeof(temp)); ret < 0) {
     ...
     */
    int Accept() {
        return Accept(_sockfd, _addr_helper);
    }
